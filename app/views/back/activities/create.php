<div class="card shadow-sm">
	<div class="card-body">
		<h2 class="mb-4">Créer une activité</h2>
		<?php if (!empty($errors)): ?>
		...
		<form method="post">
			<div class="row g-3">
				...
				<div class="col-md-4">
					...
					<input type="date" name="date_activite" class="form-control" value="<?= htmlspecialchars($old['date_activite'] ?? '') ?>">
				</div>
				<div class="col-md-4">
					<label class="form-label">Heure début</label>
					<input type="time" name="heure_debut" class="form-control" value="<?= htmlspecialchars($old['heure_debut'] ?? '') ?>">
				</div>
				<div class="col-md-4">
					<label class="form-label">Heure fin</label>
					<input type="time" name="heure_fin" class="form-control" value="<?= htmlspecialchars($old['heure_fin'] ?? '') ?>">
				</div>
				<div class="col-md-6">
					<label class="form-label">Capacité</label>
					<input type="number" name="capacite" class="form-control" min="0" value="<?= htmlspecialchars($old['capacite'] ?? '0') ?>">
				</div>
				<div class="col-md-6">
					<label class="form-label">Statut</label>
					<select name="statut" class="form-select">
						<?php $s = strtolower(trim((string)($old['statut'] ?? 'actif'))); $isActif = in_array($s, ['actif','active','1','true','oui','yes','on'], true); ?>
				...
				<div class="col-md-6">
					...
					<select name="id_entraineur" class="form-select" required>
						...
						<?php foreach ($coaches as $coach): ?>
							<option value="<?= (int)$coach['id'] ?>" <?= (isset($old['id_entraineur']) && (int)$old['id_entraineur'] === (int)$coach['id']) ? 'selected' : '' ?>>
